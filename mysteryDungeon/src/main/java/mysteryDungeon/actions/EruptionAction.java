    ...
    public void update() {
        ...
        if (target.hasPower(BurnPower.POWER_ID)) {
            ...
            if (AbstractDungeon.player.hasPower(CrushClawPower.POWER_ID)) {
                ...
                addToBot(new ReducePowerAction(AbstractDungeon.player, source,
                        AbstractDungeon.player.getPower(CrushClawPower.POWER_ID), 1));
            } else {
            ...
            if (toAll) {
                addToBot(new DamageAllEnemiesAction(AbstractDungeon.player, burnStacks, DamageType.THORNS,
                        AttackEffect.FIRE));
            } else {
            ...
            if (AbstractDungeon.player.hasPower(InfernoPower.POWER_ID)) {
                addToBot(new ReducePowerAction(AbstractDungeon.player, source,
                        AbstractDungeon.player.getPower(InfernoPower.POWER_ID), 1));
            } else {
